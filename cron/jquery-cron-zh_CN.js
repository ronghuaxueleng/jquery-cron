...
(function($) {
    ...
    var str_opt_minute = "";
    for (var i = 1;i <= 59; i++) {
        str_opt_minute += "<option value='"+i+"'>" + i + "</option>";
    }

    var str_opt_hour = '';
    for (var i = 1;i <= 23; i++) {
        str_opt_hour += "<option value='"+i+"'>" + i + "</option>";
    }
    ...
    function getCurrentValue(c) {
        ...
        switch (selectedPeriod) {
            case "minute":
                dminute = b["dminute"].find("select").val();
                min = "*/"+dminute;
                break;
            case "hour":
                dhour = b["dhour"].find("select").val();
    ...
    var methods = {
        init : function(opts) {
            ...
            block["text"] = $("<span>每 </span>").appendTo(this).data("root", this);
            block["dminute"] = $("<span class='cron-block'>"
                    +"<select class='cron-period-opt cron-period-minute'>"
                    +str_opt_minute+"</select> </span>")
                .appendTo(this)
                .data("root", this);
            block["dminute"].find("select").data("root", this);

            block["dhour"] = $("<span class='cron-block'>"
                    +"<select class='cron-period-opt cron-period-hour'>"
                    +str_opt_hour+"</select> </span>")
                .appendTo(this)
                .data("root", this);
            block["dhour"].find("select").data("root", this);
            ...
            if (defined(o.customValues) && o.customValues.hasOwnProperty(t)) {
                ...
                for (var i = 0; i < targets.length; i++) {
                    ...
                    if (tgt == "dminute") {
                        var mins = v["mins"];
                        var regexp = /\d{1,2}/;
                        var m = mins.match(regexp);
                        if (m) {
                            var btgt = block[tgt].find('select.cron-period-minute').val(m[0]);
                        }else {
                            var btgt = block[tgt].find('select.cron-period-minute').val(1);
                        }
                        if (useGentleSelect) btgt.gentleSelect("update");
                    } else if (tgt == "dhour") {
                        var hour = v["hour"];
                        var regexp = /\d{1,2}/;
                        var m = hour.match(regexp);
                        if (m) {
                            var btgt = block[tgt].find('select.cron-period-hour').val(m[0]);
                        }else {
                            var btgt = block[tgt].find('select.cron-period-hour').val(1);
                        }
                        if (useGentleSelect) btgt.gentleSelect("update");
                    } else if (tgt == "time") {
